...
func _physics_process(delta: float) -> void:
	...
	if shoot:
		$Sprite2D/AnimationPlayer.play("machine_gun_joe_shoot")
	else:
		$Sprite2D/AnimationPlayer.play("machine_gun_joe_idle")
	
	# Add the gravity.
	if not is_on_floor():
		velocity += get_gravity() * 0.03
		$Sprite2D/AnimationPlayer.play("machine_gun_joe_jump")
